...
该命令会进入 WSL 并执行 `scripts/wsl-ci.sh`，内容与 GitHub Actions Ubuntu job 对齐：
...
```powershell
.\scripts\run-ci-wsl.ps1 -Distro Ubuntu -Python python3.13 -InstallSystemDeps
```
...
This enters WSL and runs `scripts/wsl-ci.sh`, mirroring the GitHub Actions Ubuntu job:
...
Select a distro or Python executable:
