...
**状态**：已接受 | **日期**：2026-04 | **决策者**：aicoreutils 团队

#### 背景

aicoreutils 是为 LLM Agent 设计的 CLI 层，不是给人用的 shell。
人类导向的 CLI 输出（颜色、进度条、交互式提示、可变宽度列）对机器来说是噪音。
GNU Coreutils 以人类用户为目标。

#### 决策

aicoreutils 的所有输出默认为 JSON。--raw 标志显式绕过 JSON 用于管道组合。
...
GNU 工具默认允许这些操作（信任用户）。aicoreutils 不能信任 Agent。
...
aicoreutils 没有强制运行时依赖。仅使用标准库。测试/开发依赖（pytest、hypothesis、
...
aicoreutils 定义了 10 个语义退出码（0-8、10）。码 8 为安全阻止保留。
...
**Status**: accepted | **Date**: 2026-04 | **Deciders**: aicoreutils team

## Context

aicoreutils is designed as a CLI layer for LLM agents, not human users.
...
All aicoreutils output is JSON by default.  `--raw` flag explicitly
...
operations by design (trust the user).  aicoreutils cannot trust the
agent.
...
aicoreutils has zero mandatory runtime dependencies.  Only the standard
...
aicoreutils defines 10 semantic exit codes (0-8, 10).  Code 8 is
