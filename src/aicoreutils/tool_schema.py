...
_COMMAND_DESCRIPTIONS: dict[str, str] = {
    ...
    "basenc": "Encode or decode data in base16 (hex), base32, base64, or base64url from files or stdin. Read-only. Returns JSON with encoded/decoded data by default; use --raw for raw bytes on stdout. Select format with --base (default base64), switch to decode mode with --decode. Use --max_output_bytes to bound output size. Prefer 'basenc' when you need flexible base selection via a single tool; use 'base64' or 'base32' for fixed-format operations. See also 'base64', 'base32', 'hash' for hashing.",
    ...
    "ginstall": "Copy files and set attributes like permissions and ownership — GNU-compatible alias for 'install'. Destructive: creates or overwrites target files, makes directories with --directory/--parents, and changes metadata (default mode 755). Use --dry_run for safe preview. Prefer 'ginstall' when GNU install semantics are needed, otherwise use 'install'. See also 'install', 'cp'.",
    ...
    "install": "Copy files and set attributes (permissions, ownership) to destination. Destructive: creates files and directories, overwrites existing targets with --allow_overwrite, sets mode (default 755). Use --dry_run for safe preview. Prefer 'install' for software deployment and Makefile targets; use 'cp' for simple copying without permission changes. See also 'cp', 'ginstall'.",
    ...
    "mkfifo": "Create named pipes (FIFOs) for inter-process communication. Destructive: creates a special file that blocks readers until a writer connects (and vice versa). Fails safely if the path already exists. Use --dry_run to preview without touching the filesystem. Use --mode to set permissions (default 666), --parents to auto-create missing parent directories. Returns JSON with the created path, mode, and status on success; on error, returns structured JSON with exit code and message on stderr. Use 'mkfifo' when you need a FIFO pipe for shell-style IPC between processes. Not for creating regular files — use 'touch'. Not for regular directories — use 'mkdir'. Not for device nodes — use 'mknod'. See also 'mknod', 'mkdir', 'touch'.",
    ...
    "stdbuf": "Run a command with controlled stdout/stderr/stdin buffering: 0=none (unbuffered), L=line-buffered, or a byte size. Executes as a subprocess, captures bounded stdout/stderr, and enforces a safety timeout. Use --dry_run to preview without execution. Defaults to system buffering when no mode is set. Use to diagnose buffering-related output delays or ordering issues in pipelines. Prefer 'stdbuf' for I/O buffering control, 'nice' for CPU priority, and 'timeout' for execution time limits.",
