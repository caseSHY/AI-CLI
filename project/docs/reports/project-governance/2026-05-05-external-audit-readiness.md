...
> 重要：本报告是时间点快照，不是当前状态权威来源。当前状态以 `project/docs/status/CURRENT_STATUS.md` 为准。

### 1. 审计入口

| 领域 | 入口 | 说明 |
|---|---|---|
| 当前状态 | `project/docs/status/CURRENT_STATUS.md` | 当前事实来源。 |
| 安全模型 | `project/docs/reference/SECURITY_MODEL.md` | cwd sandbox、危险命令门控、MCP 安全控制。 |
| 生产安全 | `project/docs/guides/PRODUCTION_SECURITY.md` | MCP 部署和最低权限建议。 |
| GNU 兼容性 | `project/docs/audits/GNU_COMPATIBILITY_AUDIT.md` | GNU 子集边界和缺口。 |
| 开发审计计划 | `project/docs/development/DEVELOPMENT_AUDIT_PLAN.md` | P0-P6 工作计划。 |
| 命令测试矩阵 | `project/docs/development/COMMAND_TEST_MATRIX.md` | 风险等级和测试通道。 |
| 命令规格试点 | `project/docs/development/COMMAND_SPEC_PILOT.md` | command spec 原型。 |
| 发布治理 | `project/docs/development/RELEASE_GOVERNANCE.md` | 发布 gate 和回滚策略。 |
...
python -m pytest project/tests/test_sandbox_escape_hardening.py project/tests/test_mcp_security.py -v --tb=short
```
...
> Important: this report is a point-in-time snapshot, not the authoritative current status source. Use `project/docs/status/CURRENT_STATUS.md` for current status.

### 1. Audit Entry Points

| Area | Entry | Notes |
|---|---|---|
| Current status | `project/docs/status/CURRENT_STATUS.md` | Current factual source. |
| Security model | `project/docs/reference/SECURITY_MODEL.md` | cwd sandbox, dangerous command gates, MCP security controls. |
| Production safety | `project/docs/guides/PRODUCTION_SECURITY.md` | MCP deployment and least-privilege guidance. |
| GNU compatibility | `project/docs/audits/GNU_COMPATIBILITY_AUDIT.md` | GNU subset boundaries and gaps. |
| Development audit plan | `project/docs/development/DEVELOPMENT_AUDIT_PLAN.md` | P0-P6 work plan. |
| Command test matrix | `project/docs/development/COMMAND_TEST_MATRIX.md` | Risk levels and test lanes. |
| Command spec pilot | `project/docs/development/COMMAND_SPEC_PILOT.md` | Command spec prototype. |
| Release governance | `project/docs/development/RELEASE_GOVERNANCE.md` | Release gate and rollback policy. |
