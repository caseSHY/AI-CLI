...
动态事实（版本号、测试通过数、CI 状态、命令数、覆盖率）以 `project/docs/status/CURRENT_STATUS.md` 为准。本文件只定义工作安排和验收门槛。
...
3. 执行完整本地验证。
4. 生成一份基线审计记录，存入 `project/docs/reports/project-governance/`。

**命令**

```powershell
$env:PYTHONPATH = "src"
python -m pytest project/tests/ tests -v --tb=short
python -m ruff check src/ project/tests/
python -m ruff format --check src/ project/tests/
...
```powershell
$env:PYTHONPATH = "src"
python -m pytest project/tests/test_mcp_security.py project/tests/test_mcp_server_unit.py -v --tb=short
python -m pytest project/tests/test_sandbox_escape_hardening.py -v --tb=short
```
...
git diff -- project/docs/status/CURRENT_STATUS.md
python -m pytest project/tests/test_docs_bilingual.py project/tests/test_docs_governance.py tests/test_version_consistency.py -v --tb=short
```
...
```powershell
$env:PYTHONPATH = "src"
python -m pytest project/tests/ tests/test_version_consistency.py -v --tb=short --cov=src/aicoreutils --cov-report=term-missing
...
```powershell
$env:PYTHONPATH = "src"
python -m pytest project/tests/test_cli_black_box.py project/tests/test_golden_outputs.py project/tests/test_mcp_server.py -v --tb=short
python -m aicoreutils schema --pretty
...
Dynamic facts such as version, test counts, CI status, command count, and coverage must come from `project/docs/status/CURRENT_STATUS.md`. This document defines work plans and acceptance gates only.
...
4. Add a baseline audit record under `project/docs/reports/project-governance/`.
