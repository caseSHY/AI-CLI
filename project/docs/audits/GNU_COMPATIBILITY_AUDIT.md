...
`project/vendor/gnu-coreutils/coreutils-9.10/src/cu-progs.mk` 以及对应的
`project/vendor/gnu-coreutils/coreutils-9.10/src` 源码。

审计日期：2026-04-30。最近本地验证：2026-05-02。

### 结论摘要

`aicoreutils` 不是完整 GNU Coreutils 替代品。

- GNU Coreutils 基线命令数：109
- `agentutils schema` 登记命令数：114
- 与 GNU Coreutils 对应的已实现命令数：109
- `aicoreutils` 元命令/辅助命令：`catalog`、`schema`、`coreutils`、`hash`、`ginstall`、`tool-list`
...
`project/vendor/gnu-coreutils/coreutils-9.10/src/cu-progs.mk` and the corresponding
source files under `project/vendor/gnu-coreutils/coreutils-9.10/src`.

Audit date: 2026-04-30. Latest local verification: 2026-05-02.

### Summary

`aicoreutils` is not a complete GNU Coreutils replacement.

- GNU Coreutils command baseline: 109 commands
- `agentutils schema` commands: 114 commands
- Coreutils-equivalent commands implemented by `aicoreutils`: 109 commands
- `aicoreutils` metadata/helper commands: `catalog`, `schema`, `coreutils`, `hash`, `ginstall`, `tool-list`
