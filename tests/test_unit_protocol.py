...
from aicoreutils.utils._hashing import HASH_ALGORITHMS, digest_bytes, simple_sum16
from aicoreutils.utils._numfmt import IEC_UNITS, SI_UNITS, format_numfmt_value, parse_numfmt_value
from aicoreutils.utils._printf import coerce_printf_value, format_printf, printf_conversions
from aicoreutils.utils._ranges import alpha_suffix, numeric_suffix, parse_ranges, selected_indexes
from aicoreutils.utils._system import (
    active_user_entries,
...
from aicoreutils.utils._text import (
    count_words,
...
class SystemProtocolTests(unittest.TestCase):
    ...
    def test_resolve_user_id_name_success(self) -> None:
        ...
        with mock.patch("aicoreutils.utils._system.importlib.import_module", return_value=pwd_module):
    ...
    def test_resolve_user_id_name_unavailable_raises(self) -> None:
        ...
        with (
            mock.patch(
                "aicoreutils.utils._system.importlib.import_module",
    ...
    def test_resolve_user_id_name_missing_raises(self) -> None:
        ...
        with (
            mock.patch("aicoreutils.utils._system.importlib.import_module", return_value=pwd_module),
            self.assertRaises(AgentError),
        ):
            resolve_user_id("alice")
    ...
    def test_resolve_group_id_name_success(self) -> None:
        ...
        with mock.patch("aicoreutils.utils._system.importlib.import_module", return_value=grp_module):
    ...
    def test_resolve_group_id_name_missing_raises(self) -> None:
        ...
        with (
            mock.patch("aicoreutils.utils._system.importlib.import_module", return_value=grp_module),
            self.assertRaises(AgentError),
        ):
            resolve_group_id("staff")
    ...
    def test_run_subprocess_capture_not_found_raises(self) -> None:
        ...
        with (
            mock.patch("aicoreutils.utils._system.subprocess.run", side_effect=FileNotFoundError()),
    ...
    def test_run_subprocess_capture_permission_denied_raises(self) -> None:
        ...
        with (
            mock.patch("aicoreutils.utils._system.subprocess.run", side_effect=PermissionError()),
            self.assertRaises(AgentError) as ctx,
        ):
    ...
    def test_run_subprocess_capture_timeout(self) -> None:
        ...
        with mock.patch("aicoreutils.utils._system.subprocess.run", side_effect=timeout):
    ...
    def test_system_uptime_seconds_reads_proc(self) -> None:
        ...
        with mock.patch("aicoreutils.utils._system.Path", return_value=proc_uptime):
    ...
    def test_system_uptime_seconds_returns_none_when_fallbacks_fail(self) -> None:
        ...
        with (
            mock.patch("aicoreutils.utils._system.Path", return_value=proc_uptime),
            mock.patch("aicoreutils.utils._system.sys.platform", "linux"),
            mock.patch("aicoreutils.utils._system.cast", side_effect=AttributeError()),
        ):
    ...
    def test_stdin_tty_name_non_tty(self) -> None:
        ...
        with mock.patch("aicoreutils.utils._system.sys.stdin", stdin):
    ...
    def test_stdin_tty_name_success_and_oserror(self) -> None:
        ...
        with (
            mock.patch("aicoreutils.utils._system.sys.stdin", stdin),
            mock.patch("aicoreutils.utils._system.os.ttyname", return_value="/dev/pts/1", create=True),
        ):
            self.assertEqual(stdin_tty_name(), "/dev/pts/1")
        with (
            mock.patch("aicoreutils.utils._system.sys.stdin", stdin),
            mock.patch("aicoreutils.utils._system.os.ttyname", side_effect=OSError(), create=True),
        ):
            self.assertIsNone(stdin_tty_name())
...
class DigestFileTests(unittest.TestCase):
    def test_digest_file_temporary(self) -> None:
        ...
        from aicoreutils.utils._hashing import digest_file
