...
from aicoreutils.registry.plugins import reset_plugins
from aicoreutils.registry.tool_schema import _command_tools, tools_anthropic, tools_openai
...
class McpServerTests(unittest.TestCase):
    ...
    def test_tools_call_ls(self) -> None:
        ...
        try:
            ...
            proc.stdin.write(json.dumps({"jsonrpc": "2.0", "method": "notifications/initialized"}) + "\n")
            proc.stdin.flush()
            resp = _mcp_request(proc, "tools/call", {"name": "ls", "arguments": {"path": "docs"}})
            content = resp["result"]["content"][0]["text"]
            data = json.loads(content)
            self.assertTrue(data.get("ok"), data)
