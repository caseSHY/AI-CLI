...
⚠️ 当前状态：项目仍在活跃开发中。CLI 内部参数解析和 per-command flag 可能因 argparse 重构频繁变化，但 JSON envelope（`ok`, `result`, `error`, `command`, `version`）和 MCP tool schema 是稳定的。生产使用前请固定版本号 (`pip install aicoreutils==1.1.1`)。
